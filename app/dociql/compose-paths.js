...
function getExpandField(expandNotation) {
  ...
  var result = {
    field:
      subExpandIndex == -1
        ? expandNotation
        : expandNotation.substring(0, subExpandIndex),
    select:
      subExpandIndex == -1
        ? null
        : expandNotation
            .substring(subExpandIndex + 1, expandNotation.length - 1)
            .split(" "),
  };
...
module.exports = function (graphQLSchema, mutations, queries) {
  function composePath(tag, usecase) {
    ...
    if (queryTokens.length < 2)
      throw new TypeError(
        `Domain: ${tag}. Usecase query '${usecase.query}' is not well formed.\nExpected 'query.<fieldName>' or 'mutation.<mutationName>'`
      );
    const typeDict =
      queryTokens[0] == "query"
    ...
    queryTokens.forEach((token, i) => {
      if (i != 0) target = target.getFields()[token];
    });
    ...
    var examples = generateExample(
      queryTokens[0].toLowerCase(),
      target,
      expandFields
    );
