...
function generateQueryInternal(
  ...
  if (returnType.getFields) {
    ...
    keys = toSelect
      ? keys.filter((key) => toSelect.includes(key) || toExpand.includes(key))
      : keys;
...
function generateQuery(parentType, field, expandGraph) {
  ...
  const queryResult = generateQueryInternal(field, expandGraph, [], 1);
  ...
  var query = `${parentType} ${field.name}${
    argStr ? `(${argStr})` : ""
  }{\n${cleanedQuery}}`;
